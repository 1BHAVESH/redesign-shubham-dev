  ...
  return (
    ...
    >
      ...
      <nav className="bg-white w-full shadow-sm border-b">
        <div
          className="max-w-[1370px] h-16 sm:h-[70px] md:h-[78px] mx-auto 
      flex items-center justify-between px-3 sm:px-6 md:px-8 lg:px-10"
        >
          {/* LEFT MENU */}
          <ul className="hidden lg:flex gap-6 text-gray-700 text-sm font-medium">
            {menuItems.slice(0, 3).map(({ name, id, path }) => (
              <li
                key={id}
          ...
          <div className="hidden lg:flex items-center gap-6">
            <ul className="flex gap-6 text-gray-700 text-sm font-medium">
              {menuItems.slice(3).map(({ name, id, path }) => (
                <li
                  key={id}
                  onClick={() => setActiveNav(id)}
                  className={`cursor-pointer border-b-2 pb-1 transition-all whitespace-nowrap
            ...
            <Dialog>
              <DialogTrigger asChild>
                <button className="cursor-pointer bg-black text-[#d4af37] px-7 py-2 rounded-full text-sm font-medium hover:bg-gray-900 transition-colors">
                  Enquiry
                </button>
              </DialogTrigger>
              <DialogContent>
          ...
          <div className="lg:hidden flex items-center gap-3">
            <Sheet>
              ...
              <SheetContent
                side="right"
                className="bg-white w-[280px] h-[450px]"
              >
